...
{{ data.item }}
  ...
  computed: {
    ...
    pagedFilteredSortedItems() {
      // logInfo("Offers", "pagedFilteredSortedItems - results[0..1]: " + JSON.stringify(this.filteredSortedItems.slice(0, 2), null, 2));
  ...
  methods: {
    ...
    toggleTokenContractJunk(item) {
      logInfo("Offers", ".methods.toggleTokenContractJunk - item: " + JSON.stringify(item, null, 2));
      store.dispatch('data/toggleTokenContractJunk', item);
    },
    toggleTokenContractFavourite(item) {
      logInfo("Offers", ".methods.toggleTokenContractFavourite - item: " + JSON.stringify(item, null, 2));
    ...
    saveSettings() {
      logInfo("Offers", "methods.saveSettings - nftSpreadsOffersSettings: " + JSON.stringify(this.settings, null, 2));
      localStorage.nftSpreadsOffersSettings = JSON.stringify(this.settings);
    },
    ...
    rowSelected(item) {
      logInfo("Offers", "methods.rowSelected BEGIN: " + JSON.stringify(item, null, 2));
    ...
    async timeoutCallback() {
      logDebug("Offers", "timeoutCallback() count: " + this.count);
  ...
  beforeDestroy() {
    logDebug("Offers", "beforeDestroy()");
  },
  mounted() {
    logDebug("Offers", "mounted() $route: " + JSON.stringify(this.$route.params));
    store.dispatch('data/restoreState');
    if ('nftSpreadsOffersSettings' in localStorage) {
      const tempSettings = JSON.parse(localStorage.nftSpreadsOffersSettings);
    ...
    logDebug("Offers", "Calling timeoutCallback()");
...
const offersModule = {
  ...
  mutations: {
    deQueue(state) {
      logDebug("offersModule", "deQueue(" + JSON.stringify(state.executionQueue) + ")");
      state.executionQueue.shift();
    },
    updateParams(state, params) {
      state.params = params;
      logDebug("offersModule", "updateParams('" + params + "')")
    },
    updateExecuting(state, executing) {
      state.executing = executing;
      logDebug("offersModule", "updateExecuting(" + executing + ")")
    },
  },
  actions: {
  },
