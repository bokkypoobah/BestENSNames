          ...
          <b-row v-for="(item, index) of items" v-bind:key="index">
            ...
            <b-col cols="3" class="mb-1 pr-1">
              ...
              <div v-if="item.type == 'erc721'">
                <b-button size="sm" :href="'https://testnets.opensea.io/assets/sepolia/' + item.token + '/' + item.tokenId" variant="link" v-b-popover.hover.bottom="item.tokenId" class="m-0 ml-2 p-0" target="_blank">{{ item.tokenId.toString().length > 20 ? (item.tokenId.toString().substring(0, 8) + '...' + item.tokenId.toString().slice(-8)) : item.tokenId.toString() }}</b-button>
              </div>
            </b-col>
