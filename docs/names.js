const Names = {
  ...
  methods: {
    ...
    saveTokenTag(info) {
      logInfo("Names", ".methods.saveTokenTag - info: " + JSON.stringify(info, null, 2));
      store.dispatch('data/saveTokenTag', info);
    },
    toggleTokenContractJunk(item) {
      logInfo("Names", ".methods.toggleTokenContractJunk - item: " + JSON.stringify(item, null, 2));
    ...
    async requestReservoirAPITokenMetadataRefresh(token) {
      logInfo("Names", ".methods.requestReservoirAPITokenMetadataRefresh - token: " + JSON.stringify(token, null, 2));
      const options = {
        method: 'POST',
    ...
    saveSettings() {
      logInfo("Names", "methods.saveSettings - nftSpreadsTokensSettings: " + JSON.stringify(this.settings, null, 2));
