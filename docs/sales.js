...
{{ data.item }}
  ...
  computed: {
    ...
    pagedFilteredSortedItems() {
      // logInfo("Sales", "pagedFilteredSortedItems - results[0..1]: " + JSON.stringify(this.filteredSortedItems.slice(0, 2), null, 2));
  ...
  methods: {
    ...
    toggleTokenContractJunk(item) {
      logInfo("Sales", ".methods.toggleTokenContractJunk - item: " + JSON.stringify(item, null, 2));
    ...
    saveSettings() {
      logInfo("Sales", "methods.saveSettings - nftSpreadsSalesSettings: " + JSON.stringify(this.settings, null, 2));
      localStorage.nftSpreadsSalesSettings = JSON.stringify(this.settings);
    },
  ...
  mounted() {
    logDebug("Sales", "mounted() $route: " + JSON.stringify(this.$route.params));
    store.dispatch('data/restoreState');
    if ('nftSpreadsSalesSettings' in localStorage) {
      const tempSettings = JSON.parse(localStorage.nftSpreadsSalesSettings);
...
const salesModule = {
  ...
  mutations: {
    deQueue(state) {
      logDebug("salesModule", "deQueue(" + JSON.stringify(state.executionQueue) + ")");
      state.executionQueue.shift();
    },
    updateParams(state, params) {
      state.params = params;
      logDebug("salesModule", "updateParams('" + params + "')")
    },
    updateExecuting(state, executing) {
      state.executing = executing;
      logDebug("salesModule", "updateExecuting(" + executing + ")")
    },
  },
  actions: {
  },
