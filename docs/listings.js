            ...
            <b-table ref="tokenContractsTable" small fixed striped responsive hover selectable select-mode="single" @row-selected='rowSelected' :fields="fields" :items="pagedFilteredSortedItems" show-empty head-variant="light" class="m-0 mt-1">
              ...
              <template #cell(maker)="data">
                <b-link :href="'https://etherscan.io/address/' + data.item.maker" target="_blank">
                  <font size="-1">{{ nameOrAddress(data.item.maker) }}</font>
                </b-link>
              </template>

              <template #cell(taker)="data">
                <b-link :href="'https://etherscan.io/address/' + data.item.taker" target="_blank">
                  <font size="-1">{{ nameOrAddress(data.item.taker) }}</font>
                </b-link>
              </template>
...
{{ data.item }}
  ...
  computed: {
    ...
    listings() {
      return store.getters['data/listings'];
    },
    ens() {
    ...
    pagedFilteredSortedItems() {
      // logInfo("Listings", "pagedFilteredSortedItems - results[0..1]: " + JSON.stringify(this.filteredSortedItems.slice(0, 2), null, 2));
  ...
  methods: {
    ...
    toggleTokenContractJunk(item) {
      logInfo("Listings", ".methods.toggleTokenContractJunk - item: " + JSON.stringify(item, null, 2));
      store.dispatch('data/toggleTokenContractJunk', item);
    },
    toggleTokenContractFavourite(item) {
      logInfo("Listings", ".methods.toggleTokenContractFavourite - item: " + JSON.stringify(item, null, 2));
    ...
    saveSettings() {
      logInfo("Listings", "methods.saveSettings - nftSpreadsListingsSettings: " + JSON.stringify(this.settings, null, 2));
      localStorage.nftSpreadsListingsSettings = JSON.stringify(this.settings);
    },
    ...
    rowSelected(item) {
      logInfo("Listings", "methods.rowSelected BEGIN: " + JSON.stringify(item, null, 2));
    ...
    async timeoutCallback() {
      logDebug("Listings", "timeoutCallback() count: " + this.count);
  ...
  beforeDestroy() {
    logDebug("Listings", "beforeDestroy()");
  },
  mounted() {
    logDebug("Listings", "mounted() $route: " + JSON.stringify(this.$route.params));
    store.dispatch('data/restoreState');
    if ('nftSpreadsListingsSettings' in localStorage) {
      const tempSettings = JSON.parse(localStorage.nftSpreadsListingsSettings);
    ...
    logDebug("Listings", "Calling timeoutCallback()");
...
const listingsModule = {
  ...
  mutations: {
    deQueue(state) {
      logDebug("listingsModule", "deQueue(" + JSON.stringify(state.executionQueue) + ")");
      state.executionQueue.shift();
    },
    updateParams(state, params) {
      state.params = params;
      logDebug("listingsModule", "updateParams('" + params + "')")
    },
    updateExecuting(state, executing) {
      state.executing = executing;
      logDebug("listingsModule", "updateExecuting(" + executing + ")")
    },
  },
  actions: {
  },
